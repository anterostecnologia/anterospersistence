...
public class SQLPersistenceContextImpl implements SQLPersistenceContext {
	...
	private Map<EntityManaged, Object> entities = new HashMap<EntityManaged, Object>();
	...
	@Override
	public EntityManaged addEntityManaged(Object key, boolean readOnly,
			boolean newEntity) throws Exception {
		EntityManaged result = getEntityManaged(key);
		if (result == null) {
			EntityCache entityCache = entityCacheManager.getEntityCache(key
					.getClass());
			result = new EntityManaged(entityCache);
			result.setStatus(readOnly ? EntityStatus.READ_ONLY
					: EntityStatus.MANAGED);
			result.setFieldsForUpdate(entityCache.getAllFieldNames());
			result.setNewEntity(newEntity);
			for (DescriptionField descriptionField : entityCache
					.getDescriptionFields())
				result.addLastValue(descriptionField.getFieldEntityValue(
						session, key));
			entities.put(result, key);
		}
		return result;
	}
	...
	public EntityManaged getEntityManaged(Object key) {
		...
		for (EntityManaged entityManaged : entities.keySet()) {
			obj = entities.get(entityManaged);
			/*
			 ...
			 */
			if (System.identityHashCode(key) == System.identityHashCode(obj))
