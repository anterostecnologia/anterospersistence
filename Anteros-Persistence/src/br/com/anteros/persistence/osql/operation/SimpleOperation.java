...
public class SimpleOperation<T> extends SimpleCondition<T> implements Operation<T> {

	public static <D> SimpleCondition<D> create(Class<D> type, Operator<? super D> operator, Condition<?> one) {
		return new SimpleOperation<D>(type, operator, ImmutableList.<Condition<?>> of(one));
	}

	public static <D> SimpleCondition<D> create(Class<D> type, Operator<? super D> operator, Condition<?> one,
			Condition<?> two) {
		return new SimpleOperation<D>(type, operator, ImmutableList.of(one, two));
	}

	public static <D> SimpleCondition<D> create(Class<D> type, Operator<? super D> operator, Condition<?>... arguments) {
		return new SimpleOperation<D>(type, operator, arguments);
	}

	private final OperationImpl<T> sourceOperation;

	protected SimpleOperation(Class<T> type, Operator<? super T> operator, Condition<?>... arguments) {
		this(type, operator, ImmutableList.copyOf(arguments));
	}

	protected SimpleOperation(Class<T> type, Operator<? super T> operator, ImmutableList<Condition<?>> arguments) {
