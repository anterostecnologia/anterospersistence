...
public class ElementCollectionHandler implements ResultSetHandler {
	...
	public Object handle(ResultSet rs) throws Exception {
		...
		if (rs.next()) {
			collectionResult = (Collection<Object>) ReflectionUtils.getConcreteImplementationFromCollection(descriptionField.getField().getType()).newInstance();
			do {
				for (DescriptionColumn descriptionColumn : descriptionField.getDescriptionColumns()) {
					if (descriptionColumn.isElementColumn()) {
						try {
							collectionResult.add(rs.getObject(descriptionColumn.getColumnName()));
						} catch (Exception e) {
