...
public class BeanPath<T> extends SimpleExpression<T> implements Path<T> {
	...
	public BeanPath(Class<? extends T> type, PathMetadata<?> metadata, PathInits inits) {
	...
	public final <R, C> R accept(Visitor<R, C> v, C context) {
		// mixin is not used here, because subtype instances may have data that
	...
	public <U extends BeanPath<? extends T>> U as(Class<U> clazz) {
		try {
			if (!casts.containsKey(clazz)) {
				...
				U rv;
				// the inits for the subtype will be wider, if it's a variable
				// path
