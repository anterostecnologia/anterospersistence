...
public abstract class AbstractSQLSessionFactory implements SQLSessionFactory {
	...
	public void generateDDL() throws Exception {
		 ...
		 */
		String databaseDDLGeneration = configuration.getPropertyDef(AnterosProperties.DATABASE_DDL_GENERATION, AnterosProperties.NONE);
		databaseDDLGeneration = databaseDDLGeneration.toLowerCase();
		/*
		 * Verifica se é para gerar o schema em script sql
		 */
		String scriptDDLGeneration = configuration.getPropertyDef(AnterosProperties.SCRIPT_DDL_GENERATION, AnterosProperties.NONE);
		 ...
		 */
		if ((databaseDDLGeneration.equals(AnterosProperties.NONE)) && (scriptDDLGeneration.equals(AnterosProperties.NONE))) {
			return;
		}
		 ...
		 */
		Boolean createReferentialIntegrity = Boolean.parseBoolean(configuration.getPropertyDef(AnterosProperties.CREATE_REFERENCIAL_INTEGRITY, "true"));
	...
	public void generateDDL(TableCreationType databaseDDLType, TableCreationType scriptDDLType, Boolean createReferentialIntegrity) throws Exception {
		...
		if ((databaseDDLType != TableCreationType.NONE) || ((scriptDDLType != TableCreationType.NONE))) {
			 ...
			 * SQL, AMBOS
			 */
			String ddlGenerationMode = configuration.getPropertyDef(AnterosProperties.DDL_OUTPUT_MODE, AnterosProperties.DEFAULT_DDL_GENERATION_MODE);
			if (ddlGenerationMode.equals(AnterosProperties.NONE)) {
				return;
			}

			SchemaManager schemaManager = new SchemaManager(this.getSession(), entityCacheManager, createReferentialIntegrity);
			 ...
			 */
			if (ddlGenerationMode.equals(AnterosProperties.DDL_SQL_SCRIPT_OUTPUT) || ddlGenerationMode.equals(AnterosProperties.DDL_BOTH_OUTPUT)) {
			 ...
			 */
			if (ddlGenerationMode.equals(AnterosProperties.DDL_DATABASE_OUTPUT) || ddlGenerationMode.equals(AnterosProperties.DDL_BOTH_OUTPUT)) {
	...
	public void onBeforeExecuteCommit(Connection connection) throws Exception {
		if (localSession.get() != null)
			localSession.get().getPersistenceContext().onBeforeExecuteCommit(connection);
	}

	public void onBeforeExecuteRollback(Connection connection) throws Exception {
		if (localSession.get() != null)
			localSession.get().getPersistenceContext().onBeforeExecuteRollback(connection);
	}

	public void onAfterExecuteCommit(Connection connection) throws Exception {
		if (localSession.get() != null)
			localSession.get().getPersistenceContext().onAfterExecuteCommit(connection);
	}

	public void onAfterExecuteRollback(Connection connection) throws Exception {
		if (localSession.get() != null)
			localSession.get().getPersistenceContext().onAfterExecuteRollback(connection);
	}
