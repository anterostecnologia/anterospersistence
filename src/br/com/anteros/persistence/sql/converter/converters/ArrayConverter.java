...
public class ArrayConverter extends AbstractConverter {
	...
	protected Object convertToType(Class<?> type, Object value) throws Throwable {
		...
		if (!type.isArray()) {
			throw new SQLConversionException(toString(getClass()) + " cannot handle conversion to '" + toString(type) + "' (not an array).");
		}
		...
		try {
			...
			while (true) {
				...
				if ((ttype == StreamTokenizer.TT_WORD) || (ttype > 0)) {
					...
					throw new SQLConversionException("Encountered token of type " + ttype + " parsing elements to '" + toString(type) + ".");
				}
			}
			if (list == null) {
			...
			throw new SQLConversionException("Error converting from String to '" + toString(type) + "': " + e.getMessage());
		}
