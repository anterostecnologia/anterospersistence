				...
				try{
					...
					if(argCount == 0){
						if(name.startsWith(GET_PREFIX)){
							// Simple getter
							pd = new PropertyDescriptor(BeanUtils.decapitalize(name.substring(3)),
									method, null);
						} else if(resultType == boolean.class && name.startsWith(IS_PREFIX)){
							// Boolean getter
							pd = new PropertyDescriptor(BeanUtils.decapitalize(name.substring(2)),
									method, null);
						}
					} else if(argCount == 1){
						if(argTypes[0] == int.class && name.startsWith(GET_PREFIX)){
							pd = new IndexedPropertyDescriptor(BeanUtils.decapitalize(name.substring(3)),
									null, null, method, null);
						} else if(resultType == void.class && name.startsWith(SET_PREFIX)){
							// Simple setter
							pd = new PropertyDescriptor(BeanUtils.decapitalize(name.substring(3)), null,
									method);
		...
		if(propType.isArray() && propType.getComponentType() == ipropType){
			...
			if(pd.getClass0().isAssignableFrom(ipd.getClass0())){
				...
				if(read == null && write != null){
					read = BeanUtils.findAccessibleMethodIncludeInterfaces(result.getClass0(), "get"
				...
				if(write == null && read != null){
					write = BeanUtils.findAccessibleMethodIncludeInterfaces(result.getClass0(), "set"
		...
		if(explicitEvents != null){
			...
			if(adds != null && removes != null){
				...
				while(keys.hasNext()){
					...
					String eventName = BeanUtils.decapitalize(listenerName.substring(0, listenerName
							.length() - 8));
